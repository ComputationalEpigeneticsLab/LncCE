...
//cancer												
						...
						String[] sour={"cell_res","geo","other","tiger"};
						for(String i : sour){
...
String tmp_cs = tissue_pd+";"+resource1;
...
List<String> intecancer_uniq = new ArrayList<String>();
for(int j=0;j<intesour.size();j++){
...
for(int n=0;n<intesour_uniq0.size();n++){
	if(!(intesour_uniq0.get(n).equals(resource1))){
...
<div class="heading">
<h4>
1) Spatial expression pattern of <span style="color:#525acd"><%=tissue_pd %></span> tissue in the data resource of <span style="color:#525acd"><%=gse%></span>
</h4>
</div>
	
<div class="container-fluid">
<%if(resource.equals("geo")||resource.equals("tiger")){%>	
	...
	db.open();
	sel = "select * from "+resource+"_basic where ensembl_gene_id ='" + ID + "' and tissue='" + tissue_pd + "' and dataset='" + gse + "' and seqnames='"+chr+"'";
	rs = db.execute(sel);
	rs.beforeFirst();
	while(rs.next()){
	...
	out.println("<tr class=\"single\" \"font-size\">");
	out.println("<th>");
		out.println("Cancer");
	out.println("</th>");
	out.println("<th>");
...
//柱状图查询	
	db.open();
	sel = "select * from "+resource+"_"+a_f+"_lncexp where result=\""+NODE+"\" and tissue_type='"+tissue_pd+"_"+gse+"'";
	//System.out.println(sel);
	rs=db.execute(sel);
	...
	while(rs.next()){
		for(int g=0;g<oldcol.size();g++){			
			if(oldcol.get(g).equals("Monocyte or Macrophage")){
				coln.add("Monocyte/Macrophage");
...
<div class="card card-body row-distance">
  <div class="row">
      ...
      <div class="card-body" style="text-align:center;">
        ...
        <p class="card-text" style="text-align:center;"><small class="text-muted">Tissue:<%out.println(tissue_pd);%></small></p>
        <div id="bar_c" class="bar"></div>
      </div>
      </div>
  
		<script>
				var chartDom = document.getElementById('bar_c');
...
<%/*查询detail*/
					...
					rs=db.execute(sel);
						...
						while(rs.next()){
							scat_0.add(rs.getString(tissue_pd+"_"+gse+""));
						}
						rs.close();
						db.close();
						...
						rs=db.execute(sel);
						while(rs.next()){
							cancermax1=rs.getString(tissue_pd+"_"+gse+"");
						}
						rs.close();
						db.close();
...
<div class="card card-body row-distance">
  <div class="row">
      ...
      <div class="card-body" style="text-align:center;">
        ...
        <p class="card-text" style="text-align:center;"><small class="text-muted">Tissue:<%out.println(tissue_pd);%></small></p>
        <div id="cluster_c" class="cluster_e"></div>
      </div>
      </div>

<script>
var myChart = echarts.init(document.getElementById('cluster_c'));
var data_left = [
            	 <% 
...
if(ex.equals("NONE")){
	...
	<div class="col-6">
      <div class="card-body" style="text-align:center;">
        ...
        <p class="card-text" style="text-align:center;"><small class="text-muted">Tissue:<%out.println(tissue_pd);%></small></p>
        <div id="clustere_c" class="cluster_e"></div>
      </div>
    </div>
  </div>
</div>
<script>
var myChart = echarts.init(document.getElementById('clustere_c'));
var data_right = [
             	 <% 
             	 for(int p=0;p<scat0ldz1_0.length;p++){
             		  String[] scat0ldz2_0 = scat0ldz1_0[p].split(",");
...
<button id="sour_btn4" type="submit" value="Submit" class="btn btn-dark">Apply Filter</button>
</form>
</div>
<div style="height:10px;"></div>
<%
//max mean查询
if(select_other.equals("cell_res")||select_other.equals("other")){ %>
...
<%  
	...
	db.open();
	sel = "select * from "+select_other+"_basic where ensembl_gene_id ='" + ID + "' and tissue='" + slet_cancer + "' and source='"+a_f+"'";
	rs = db.execute(sel);
	rs.beforeFirst();
	while(rs.next()){
...
//柱状图查询
	...
	while(rs.next()){
		for(int g=0;g<oldcol1.size();g++){
			if(oldcol1.get(g).equals("Monocyte or Macrophage")){
				coln1.add("Monocyte/Macrophage");
...
<%/*查询detail*/
					...
					rs=db.execute(sel);
						...
						rs.beforeFirst();
						while(rs.next()){
							scat3.add(rs.getString(slet_cancer));
						}
						rs.close();
						db.close();
						...
						rs=db.execute(sel);
						while(rs.next()){
							cancermax=rs.getString(slet_cancer);
						}
						rs.close();
						db.close();
...
//initViolin(){  //渲染violin图
    ...
    let layout_other = {
      <%--title: "Expression level of <%=NODE%><br>"+'Tissue: '+<%out.println("'"+slet_cancer+"'");%>,--%>
      /*font:{
    	  family:'Microsoft YaHei',
    	  size:14
      },*/
...
//柱状图查询	
	...
	while(rs.next()){
		for(int g=0;g<oldcol2.size();g++){			
			if(oldcol2.get(g).equals("Monocyte or Macrophage")){
				coln2.add("Monocyte/Macrophage");
  ...
  <div class="row">
    <div class="col-6">
      <div class="card-body" style="text-align:center;">
        <h5 class="card-title"><b><%=NODE%> Mean Exp in celltypes</b></h5>
        <p class="card-text" style="text-align:center;"><small class="text-muted">Tissue:<%out.println(slet_cancer);%></small></p>
        <div id="bar_other1" class="bar"></div>
      </div>
    </div>
...
<%/*查询detail*/
					...
					rs=db.execute(sel);
						...
						List<String> cell_type3 = new ArrayList<String>();
						while(rs.next()){
							scat_2.add(rs.getString(slet_cancer+"_"+select_gse+""));
						}
						rs.close();
						db.close();
						...
						rs=db.execute(sel);
						while(rs.next()){
							cancermax2=rs.getString(slet_cancer+"_"+select_gse+"");
						}
						rs.close();
						db.close();
						...
						//去重细胞类型
			        	  List<String> celltype_uniq2 = new ArrayList<String>();
			        	  for(int p=0;p<cell_type2.size();p++){
			        		  celltype_uniq2.add(cell_type2.get(p).split(",")[0]);
			        	  }
...
//initViolin(){  //渲染violin图
      ...
      for(int p=0;p<sca;p++){
      	...
      	for(int q=0;q<sca2;q++){
      		out.println("{");
      			...
      			out.println("GROUP_NAME:"+"\""+celltype_uniq2.get(p)+"\""+",");
      			out.println("GROUP_SHORT:"+"\""+celltype_uniq2.get(p)+"\""+",");
      		out.println("},");;		
      	}		
      }
      %>     
    ];
    function unpack(rows_other1, key) {
      return rows_other1.map(function(row) { return row[key]; });
    }
    let data_other1 = [{
      type: 'violin',
      x: unpack(rows_other1, 'GROUP_SHORT'),
      y: unpack(rows_other1, 'EXPRESSION_VALUE'),
      //points: 'none',
      //hoverinfo: 'skip',//hover不显示数据信息
      box: {
        visible: true
      ...
      transforms: [{
        type: 'groupby',
        groups: unpack(rows_other1, 'GROUP_SHORT'),
        styles: [
          <% 
          for(int q=0;q<celltype_uniq2.size();q++){
             out.println("{target:"+"'"+celltype_uniq2.get(q)+"'"+","+"value: {line: {color: '"+volcolor[q]+"'}}},");
           }
          %>
        ],
      }]
    ...
    Plotly.newPlot('viloin_other1', data_other1, layout_other1, {showSendToCloud: true});
  //},
</script>
<%}
/*----------------------------------小提琴图end----------------------------------*/
...
var data_left = [
            ...
            var leg = [
            	<%
            	for(int q=0;q<celltype_uniq2.size();q++){
            		out.println("'" +celltype_uniq2.get(q)+"'"+",");
            	}
            	%>
            ];
            option = {       	    
