...
<div class="heading"><h4>1) Spatial expression pattern of <span style="color:#525acd"><%=end_tissue%></span> tissue in <span style="color:#525acd"><%=hash_resource.get(resource) %></span></h4></div>
<% //max mean查询
if(resource.equals("other")||resource.equals("epn")){ %>	
<%  
	...
	db.open();
	sel = "select * from "+resource+"_basic where ensembl_gene_id ='" + ID + "' and tissue='" + tissue_pd + "' and cell='"+cell+"' and source='"+a_f+"'";
	rs = db.execute(sel);
	rs.beforeFirst();
	while(rs.next()){
	...
	db.close();
	//max mean表格
	out.println("<table id='cancer-table' class='table-lyj row-distance round-border' style='width:100%'>");
...
//柱状图查询
	...
	while(rs.next()){
		for(int g=0;g<oldcol.size();g++){
			colname0.add(oldcol.get(g));
			v0.add(rs.getString(oldcol.get(g)));
		}
	}
	rs.close();
	db.close();
  ...
  <div class="row">
    <div class="col-6">
      <div class="card-body" style="text-align:center;">
        ...
        <div id="bar_c_re1" class="bar"></div>
      </div>
    </div>
...
<h4>
2) Spatial expression pattern of <%=end_tissue %> tissue in other resources
</h4>
</div>
<div>
<h6>No result!</h6>
</div>
<%}else if(intesour_uniq.size()>0){ 	
//选择的资源
String slet_sour = request.getParameter("seother");
String select_other = "";
if(slet_sour==null){	
	slet_sour = intesour_uniq.get(0);
	select_other = intesour_uniq.get(0);
}else{
	select_other = slet_sour;
}
%>
<div class="heading row-distance">
<h4>
...
<%
/*把选择的资源放在第一个*/
	out.println("<option value='"+slet_sour+"'>");
...
//max mean表格
	...
	db.open();
	sel = "select * from "+select_other+"_basic where ensembl_gene_id ='" + ID + "' and tissue='" + tissue_pd + "' and source='"+a_f+"'";
	rs = db.execute(sel);
	rs.beforeFirst();
	while(rs.next()){
	...
	//柱状图查询	
		...
		while(rs.next()){
			for(int g=0;g<oldcol2.size();g++){
				colname2.add(oldcol2.get(g));
				v2.add(rs.getString(oldcol2.get(g)));
			}
		}
		rs.close();
		db.close();	
...
<%/*查询detail*/				
					...
					rs=db.execute(sel);
						...
						while(rs.next()){
							scat_2.add(rs.getString(tissue_pd));
						}
						rs.close();
						db.close();
						...
						rs=db.execute(sel);
						while(rs.next()){
							cancermax2=rs.getString(tissue_pd);
						}
						rs.close();
						db.close();
