...
function submit2(){
	document.form2.action = 'tissue_details.jsp?#floatMenu3';
...
String end_setiss = ((select_tiss.replace("NA_","")).replace("_"," ")).replace("-"," ");
%>
<div>
<form id="form2" name="form2">
<input type="hidden" name="value1" value=0.5>
<input type="hidden" name="tissue" value='<%=tissue_pd %>'>
<input type="hidden" name="mRNA_tissue" value='<%=tissue_pd %>'>
...
out.println(end_setiss+" ("+hash_resource.get(select_sour)+")");		
out.println("</option>");
//循环输出所有的组织+资源
...
<button type="submit" value="Submit" class="btn btn-dark" onclick="submit2()">Apply Filter</button>
</form>
</div>

<div class="heading row-distance"><h4>Spatial expression pattern of <%=end_setiss%> tissue in the data resource of <span style="color:#525acd"><%=hash_resource.get(select_sour)%></span></h4></div>
<%
/*max mean 表格*/
//----max mean normal----
