...
String end_tiss = ((select_tiss.replace("NA_","")).replace("_"," ")).replace("-"," ");
%>
<div>
...
for(int se=0;se<intesour_uniqcan.size();se++){		
		...
		String end_inun = ((inun[0].replace("NA_","")).replace("_"," ")).replace("-"," ");
		if(inun.length>2){
			out.println("<option value='"+intesour_uniqcan.get(se)+"'>");
			out.println(end_inun+" ("+inun[2]+")");
			out.println("</option>");
		}else{
			out.println("<option value='"+intesour_uniqcan.get(se)+"'>");
			out.println(end_inun+" ("+hash_resource.get(inun[1])+")");
			out.println("</option>");
		}
	}
%>
...
<div class="heading">
<h4>
Spatial expression pattern of <%=end_tiss %> tissue in the data resource of <span style="color:#525acd"><%=hash_resource.get(select_sour) %></span>
</h4>
</div>
<%}%>
...
<%  	
	...
	for(int j=0;j<cesize;j++){
		...
		//out.println("<span title=\""+hash_fullna.get(clasii.get(j))+" : "+hash_description.get(clasii.get(j))+"\" class=\"glyphicon glyphicon-question-sign\"></span>");						
...
//initViolin(){  //渲染violin图
    ...
    Plotly.newPlot('viloinc', data_1, layout_1, {showSendToCloud: true});
  //},
</script>
<!-- 箱线图 end -->
<%} %>
