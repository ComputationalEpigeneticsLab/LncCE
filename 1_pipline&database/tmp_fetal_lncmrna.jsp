...
function actsel1(s) {
	if (s != "") {
		document.form1.action = 'tmp_fetal_lncmrna.jsp?#abc';
...
<form action="tmp_fetal_lncmrna.jsp" name="form1">
			<table id="lnc_mrna" class="table-lyj round-border display" width="100%">
			<thead>
			<tr class="single">
			<th style="width: 120px;text-align: left;">Name</th>
			<th style="text-align: left;"><div class="row">&nbsp&nbsp&nbspTissue&nbsp&nbsp
			<%-- 
			...
			out.println("<option value='"+mRNA_tissue+"'>"+a_f2+((mRNA_tissue.replace("NA_","")).replace("_"," ")).replace("-"," ")+"</option>");
			} %>
			<option value="All_tissue">all tissue</option>
			<%
			for(String t:alone_tissue){
				out.println("<option value='"+t+"'>"+a_f2+((t.replace("NA_","")).replace("_"," ")).replace("-"," ")+"</option>");
			}
			%>
 			</select>
 			--%>
...
for(int at=0;at<alone_tissue.size();at++){
	...
	for(int n4=0;n4<set1.length;n4++){		
		...
		String end_tiss = ((alone_tissue.get(at).replace("NA_", "")).replace("_"," ")).replace("-"," ");
		if(mRNA_classification.equals("All")){
			//无组织，无分类
			if(result[3].compareTo(i4) > 0){//判断R值
			out.println("<tr>");
			out.println("<td>"+result[1]+"</td>");             
            out.println("<td>"+end_tiss+"</td>");
...
//有组织
	...
	for(int n4=0;n4<set1.length;n4++){		
		...
		String end_tiss = ((mRNA_tissue.replace("NA_","")).replace("_"," ")).replace("-"," ");
		if(mRNA_classification.equals("All")){
			//有组织，无分类
			if(result[3].compareTo(i4) > 0){//判断R值
			out.println("<tr>");
